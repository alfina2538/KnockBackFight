...
type WinFlag = (typeof WinFlag)[keyof typeof WinFlag];
    ...
    system.run(() => {
      if (Playeres.CheckTpPoint()) {
        if (OVER_WORLD.getBlock(StartButton)?.typeId !== "stone_button") {
          OVER_WORLD.runCommandAsync(
            `setblock ${StartButton.x} ${StartButton.y} ${StartButton.z} stone_button`,
          );
        }
      } else {
        if (OVER_WORLD.getBlock(StartButton)?.typeId !== "air") {
          OVER_WORLD.runCommandAsync(
            `setblock ${StartButton.x} ${StartButton.y} ${StartButton.z} air`,
          );
        }
      }
    });
